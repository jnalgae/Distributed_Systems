...
echo "[third_4node_2gpu] Building analytical backend"
"${PROJECT_DIR}/build/astra_analytical/build.sh"

echo "[third_4node_2gpu] Generating synthetic LLM Chakra traces"
python3 "${SCRIPT_DIR}/scripts/generate_llm_4node_2gpu.py" --output-dir "${WORKLOAD_DIR}"
...
run_case() {
  ...
  echo "[third_4node_2gpu] Running ${case_name}"
  "${ASTRA_SIM}" \
  ...
  printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    ...
    "${throughput}" "${raw_log}" >> "${SUMMARY}"
}

for microbatches in 1 2 4 8; do
  for mode in tp2pp4 tp4pp2 tp8 pp8; do
    label=$(echo "${mode}" | tr '[:lower:]' '[:upper:]')
    run_case "${mode}_mb${microbatches}_nvlink_intra" "${label}" "NVLink-intra" "${microbatches}" "${mode}_mb${microbatches}" "${SCRIPT_DIR}/configs/network_nvlink_intra_4node_2gpu.yml"
    run_case "${mode}_mb${microbatches}_pcie_intra" "${label}" "PCIe-intra" "${microbatches}" "${mode}_mb${microbatches}" "${SCRIPT_DIR}/configs/network_pcie_intra_4node_2gpu.yml"
  done
done

echo "[third_4node_2gpu] Summary written to ${SUMMARY}"
cat "${SUMMARY}"
